...
Ensemble Runner остаётся работающим экспериментальным compatibility runtime.
Его job JSON, manifest, порядок стадий и `final_results/` описаны ниже как
текущее поведение и не являются каноническими контрактами ONYX v1.

Phase 1A добавляет JobSpec v1 и Manifest v1, но пока не подключает их к runner и
не меняет его выполнение. Целевая семантика описана в
[[JobSpec and Manifest v1]] и [[Pipeline Architecture]]. Исторические Ensemble
jobs должны читаться через compatibility importer, а не переписываться.

## Текущая runtime-архитектура
...
# Final results текущего runtime
...
В канонической модели наличие файла в `final_results/` само по себе не означает
client-ready. Авторитетная цепочка требует выбранный `IdentityResult`, успешный
postprocessing и успешный `DeliveryResult`.
...
Каноническая детализация этой схемы теперь зафиксирована в JobSpec/Manifest v1:
автоматическая QA и human review являются разными evidence, единицей отбора
служит `IdentityResult`, а human review с `client_ready=true` обязателен.
