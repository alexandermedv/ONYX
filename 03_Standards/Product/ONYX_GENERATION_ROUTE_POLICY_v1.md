...
## Boudoir route continuation rule

For Boudoir collections, a refusal or block on one scene MUST NOT terminate the entire generation round if other planned scenes can still be generated compliantly.

If a `CHATGPT_IMAGEGEN` scene is blocked:

1. Record the original blocked prompt and result.
2. Attempt up to two compliant rewrites.
3. Preserve scene intent, Boudoir aesthetics, sensuality direction, wardrobe direction, and framing intent.
4. Move toward previously documented successful ONYX Boudoir scene patterns and wording; do not silently convert the scene into generic lifestyle or fully clothed fashion.
5. If both rewrites remain blocked, mark only that scene `ROUTE_BLOCKED` and continue the remaining planned scenes.
6. Do not abort the whole round because of one blocked scene, reduce the collection's agreed sensuality/coverage, or switch route without owner approval.

Continue while enough suitable planned scenes remain to assemble the target final set. Use collection-level `ROUTE_BLOCKED` only when the route cannot produce enough suitable candidates or otherwise cannot satisfy the agreed creative brief. If fewer than the target remain, record `INSUFFICIENT_VALID_CANDIDATES`; never fill the gap with rejected/superseded assets.

Do not retry repeatedly or rephrase a refused request merely to evade route moderation. Each rewrite must be meaningfully compliant, retain approximate coverage, and be logged with its result.

For Boudoir collection QA, evaluate `sensuality_match`, `coverage_match`, `wardrobe_match`, and `boudoir_authenticity` explicitly. A frame that reads as ordinary fashion/lifestyle and is not an intentional contrast in the approved plan fails `boudoir_authenticity`. Collection QA cannot PASS unless all four checks PASS.
